\def\mytitle{BLUETOOTH AND WIFI CONTROLLED UGV WITH SEVEN SEGMENT DISPLAY}
\def\myauthor{B Sai Sindhu}
...
\item After uploading the code to the vaman board as per the given instructions, then download the Dabble apk and install it on the Android Mobile. 
\item In Dabble App.Select gamepad option in the app and then select joystick mode.
...
\item Now download the WifiToyCar apk and install it on the Android Mobile.After uploading the code to the Vaman board,Esp32 is connected to the mobile using Hotspot.
\item Now using IP Address of the Esp32,the WifiToyCar application is connected to the Esp32.Now the Seven Segment Display is controlled using the buttons in the WifiToyCar application.
